...
def contact_us(req):
    ...
    existing_record = col_feedback.find_one({"email": email})
    if existing_record:
        col_feedback.update_one({"email": email}, {"$set": {"name": name, "subject": subject, "message": message}})
    else:
        new_record = {"name": name, "email": email, "subject": subject, "message": message}
        col_feedback.insert_one(new_record)
    return render(req, "contact.html")

def contactad(req):
	contactData = list(col_feedback.find())
	return render(req,"contactad.html",{'data':contactData})
    ...
    if newpassword == cnewpassword:
        ...
        if user:
            if otp == '1957':
                ...
                return render(req, "recover.html", {"msg": "OTP is not correct, New"})
        else:
            return render(req, "recover.html")
    else:
        return render(req, "recover.html", {"msg": "Wrong credentials, New"})
